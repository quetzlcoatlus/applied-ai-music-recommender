...
def score_song(user_prefs: Dict, song: Dict) -> Tuple[float, str]:
    ...
    # Weights redistributed equally (+0.04 each) after mood (0.20) was temporarily removed.
    # Restore: genre=0.15, mood=0.20, energy=0.20, valence=0.15, danceability=0.15, acousticness=0.15
    overall_score = (
        0.19 * genre_score +
        # 0.20 * mood_score +
        0.24 * energy_score +
        0.19 * valence_score +
        0.19 * danceability_score +
        0.19 * acousticness_score
    )

    explanation = (
        f"genre={genre_score:.2f}, "
